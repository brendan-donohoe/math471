...
program homework7
  ...
  real(kind = 8), parameter :: dt = 0.000001 ! dt: The stepsize in time used in time discretization.
  integer, parameter :: nsteps = 500000 !1000 !Check at t = 0.8: Prev value: ~-15 compared to -0.72
  ...
  real(kind = 8) :: t
  real(kind = 8) :: error
  integer :: int_sum,frame
  CHARACTER(7) :: charit, t_it
  ...
  do it = 1,nsteps
    ...
    if (mod(it, 100) == 0) then
      WRITE(charit,"(I0)") myid
      WRITE(t_it, "(I0)") frame
      call printdble2d(x(1:nrl,1:nsl),y(1:nrl,1:nsl),u(1:nrl,1:nsl),nrl,nsl,'t'//trim(t_it)//'up'//trim(charit)//'.txt')
      frame = frame + 1
    end if
  end do
  ...
  !write(*,*) "Approx =u(", x(10,10), ",", y(10,10), ",", t, ")=", u(10,10)
  !write(*,*) "Exact  =", sin(2.d0*(dt*dble(it))+acos(-1.d0)-x(10,10))*sin(y(10,10)), ")" 
...
subroutine printdble2d(x,y,u,nx,ny,str)
  implicit none
  integer, intent(in) :: nx,ny
  real(kind = 8), intent(in) :: u(nx,ny),y(nx,ny),x(nx,ny)
  character(len=*), intent(in) :: str
  integer :: i,j
  open(2,file=trim(str),status='unknown')
  do j=1,ny,1
     do i=1,nx,1
        !write(2,fmt='(E24.16)',advance='no') x(i,j),y(i,j),u(i,j)
