...
program homework7
  ...
  integer, parameter :: nsteps = 100 !1000 !Check at t = 0.8: Prev value: ~-15 compared to -0.72
  ...
  do i = 1,nsl
     s(i) = -1.d0 + dble(i-1+iy_off)*hs ! Same, but for y-coordinates. I'M NOT SURE IF THIS IS RIGHT
  end do
  ...
  call MPI_Sendrecv(x(1,0:nsl+1),nsl+2,MPI_DOUBLE_PRECISION,p_left,123,&
       x(nrl+1,0:nsl+1),nsl+2,MPI_DOUBLE_PRECISION,p_right,123,MPI_COMM_WORLD,status,ierr) !CHECK
  ...
  !call MPI_Sendrecv(x(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_up,127,&
  !     x(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_down,127,MPI_COMM_WORLD,status,ierr)
  !call MPI_Sendrecv(y(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_up,128,&
  !     y(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_down,128,MPI_COMM_WORLD,status,ierr)
  ...
  !call MPI_Sendrecv(x(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_down,129,&
  !     x(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_up,129,MPI_COMM_WORLD,status,ierr)
  !call MPI_Sendrecv(y(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_down,130,&
  !     y(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_up,130,MPI_COMM_WORLD,status,ierr)
  ...
  !write(*,*) "P", myid, "Px", px!, "R", r
  !write(*,*) "P", myid, "Py", py!, "S", s

  !write(*,*) "py_max: ", py_max, "px_max: ", px_max

  
  !write(*,*) "ACTUAL BOUNDARY VALUES FOR EACH PROCESS (XR-COORDS):"
  !write(*,*) "P", myid, "Left:", xr(1,5)!x(1,1:nsl)
  !write(*,*) "P", myid, "Right:", xr(nrl,5)!x(nrl,1:nsl)
  !write(*,*) "P", myid, "Up:", xr(5,nsl)!x(1:nrl,nsl)
  !write(*,*) "P", myid, "Down:", xr(5,1)!x(1:nrl,1)
  !call MPI_BARRIER(MPI_COMM_WORLD, ierr)

  !write(*,*) "ADJACENT BOUNDARY VALUES FOR EACH PROCESS (XR-COORDS):"
  !write(*,*) "P", myid, "Left:", xr(0,5)!x(0,1:nsl)
  !write(*,*) "P", myid, "Right:", xr(nrl+1,5)!x(nrl+1,1:nsl)
  !write(*,*) "P", myid, "Up:", xr(5,nsl+1)!x(1:nrl,nsl+1)
  !write(*,*) "P", myid, "Down:", xr(5,0)!x(1:nrl,0)
  !call MPI_BARRIER(MPI_COMM_WORLD, ierr)

  !write(*,*) "CHECKING X AND Y COORDINATES:"
  !do i = 1,nrl
  !  do j = 1,nsl
  !  end do
  !end do
  ...
  write(*,*) "Approximate Error at t = ", t, "= ", error
