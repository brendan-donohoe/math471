...
program homework7
  ...
  real(kind = 8), parameter :: dt = 0.001 ! dt: The stepsize in time used in time discretization.
  ...
  !write(*,*) "u(", x(10,10), ",", y(10,10), ",0)=", um(10,10)
  !write(*,*) "u(", x(10,10), ",", y(10,10), ",", dt, ")=", u(10,10)
  ! Set up a.
  call set_a(a,nxl,nyl,x,y,manutype)
  write(*,*) "a and boundary conditions set!  Running code!"
  ! Allocate memory for the Force/rhside/Lap arrays.
  allocate(Force(0:nrl+1,0:nsl+1),rhside(0:nrl+1,0:nsl+1),utt(0:nrl+1,0:nsl+1),Lap(0:nrl+1,0:nsl+1))
  ...
  write(*,*) "u(", x(10,10), ",", y(10,10), ",", t+1*dt, ")=", up(10,10)
...
subroutine compute_forcing(Force,nxl,nyl,x,y,t,force_type)
  ...
  real(kind = 8) :: omega,t0,kx,ky
  real(kind = 8), dimension(0:nxl+1,0:nyl+1) :: a,ax,ay,ux,uy,uxx,uyy,utt
  real(kind = 8), parameter :: pi = acos(-1.d0)
  ...
  Force = utt - (ax*ux + a*uxx) - (ay*uy + a*uyy)
end subroutine compute_forcing

subroutine compute_lap(Lap,u,a,Jac,rx,sx,ry,sy,nxl,nyl,hr,hs)
  implicit none
  ...
  real(kind = 8), dimension(0:nxl+1,0:nyl+1,1:8) :: q,term
  integer :: i,j
