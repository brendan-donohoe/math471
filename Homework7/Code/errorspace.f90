program erroranalysis
  ...
  ! Compute the error at t = 1 for different step sizes.
  do n = 30, 30
    call computeerror(n,n,1d0/dble(1000+1),1000,myid,nprocs,ierr)
  end do

  call mpi_finalize(ierr)
end program erroranalysis

subroutine computeerror(nx,ny,dt,nsteps,myid,nprocs,ierr)
  use mpi
  use xycoord
  implicit none
  ...
  integer :: px,ix_off,py,iy_off ! what is my x-proc index and what is my offset
  integer :: p_left,p_right,px_max
  integer :: p_down,p_up,py_max
  integer :: nxl,nyl !local size
  integer :: remx,remy
  integer :: test_px_max,test_py_max
  real(kind = 8) :: hr,hs
  ...
  real(kind = 8) :: t
  real(kind = 8) :: error, max_error, jmax, true_jmax
  integer :: int_sum
  ...
  px_max = nprocs
  py_max = 1
  do test_px_max = 1,nprocs
    do test_py_max = 1,nprocs
      if (test_px_max*test_py_max == nprocs) then
        if(abs(nx/test_px_max-ny/test_py_max) < abs(nx/px_max-ny/py_max)) then
          px_max = test_px_max
          py_max = test_py_max
        endif
      endif
  ...
  if(myid == 0) then
     if (nx*py_max .ne. int_sum) then ! If the sum of everyone's number of x-gridpoints is not equal to the total number of gridpoints, stuff done went wrong!
  ...
  nyl = ny/py_max ! Number of y-gridpoints we're responsible for on the square domain.
  remy = ny-nyl*py_max
  if (py .le. remy) then
      nyl = nyl + 1
      iy_off = (py-1)*nyl
  ...
  call MPI_Reduce(nyl,int_sum,1,&
       MPI_INTEGER,MPI_SUM,&
       0,MPI_COMM_WORLD,ierr)
  if(myid == 0) then
      if(ny*px_max .ne. int_sum) then
          write(*,*) 'Something is wrong with the number if points in the y-direction: ',&
              ny,int_sum
      end if
  end if
  nsl = nyl ! Number of s-gridpoints we're responsible for on the square domain.

  ! who are my neighbours? 
  ! NOTE THAT WE ARE MAPPING TO THE 1D INDEXING HERE!!!!
  p_left  = myid - 1 ! ID of the process in control of the domain to our left.
  p_right = myid + 1 ! ID of the process in control of the domain to our right.
  p_up = myid + px_max
  p_down = myid - px_max
  ...
  if (py .eq. 1) p_down = MPI_PROC_NULL
  if (py .eq. py_max) p_up = MPI_PROC_NULL
  ...
  do i = 1,nsl
     s(i) = -1.d0 + dble(i-1+iy_off)*hs ! Same, but for y-coordinates.
  end do
  ...
  if(py .eq. py_max) then
     x(:,nsl+1) = 2.d0*x(:,nsl)-x(:,nsl-1)
     y(:,nsl+1) = 2.d0*y(:,nsl)-y(:,nsl-1)
  end if
  ...
  ! send to the up recieve from the down
  call MPI_Sendrecv(x(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,129,&
       x(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,129,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(y(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,130,&
       y(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,130,MPI_COMM_WORLD,status,ierr)
  ! send to the down recieve from the up
  call MPI_Sendrecv(x(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,127,&
       x(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,127,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(y(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,128,&
       y(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,128,MPI_COMM_WORLD,status,ierr)

  call MPI_BARRIER(MPI_COMM_WORLD,ierr) ! Communicate between the processes and send data between process domains.

  ! Now compute the metric TODO
  ...
  ! At a boundary we extrapolate TODO
  if(px .eq. 1) then
     xr(0,:) = 2.d0*xr(1,:)-xr(2,:)
     xs(0,:) = 2.d0*xs(1,:)-xs(2,:)
     yr(0,:) = 2.d0*yr(1,:)-yr(2,:)
  ...
  if(py .eq. py_max) then
     xr(:,nsl+1) = 2.d0*xr(:,nsl)-xr(:,nsl-1)
     xs(:,nsl+1) = 2.d0*xs(:,nsl)-xs(:,nsl-1)
     yr(:,nsl+1) = 2.d0*yr(:,nsl)-yr(:,nsl-1)
     ys(:,nsl+1) = 2.d0*ys(:,nsl)-ys(:,nsl-1)
  end if
  ...
  ! send to the down recieve from the up
  call MPI_Sendrecv(xr(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,227,&
       xr(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,227,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(yr(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,228,&
       yr(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,228,MPI_COMM_WORLD,status,ierr)
  ! send to the up recieve from the down
  call MPI_Sendrecv(xr(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,229,&
       xr(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,229,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(yr(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,230,&
       yr(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,230,MPI_COMM_WORLD,status,ierr)
  ! send to the down recieve from the up
  call MPI_Sendrecv(xs(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,327,&
       xs(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,327,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(ys(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,328,&
       ys(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,328,MPI_COMM_WORLD,status,ierr)
  ! send to the up recieve from the down
  call MPI_Sendrecv(xs(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,329,&
       xs(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,329,MPI_COMM_WORLD,status,ierr)
  call MPI_Sendrecv(ys(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,330,&
       ys(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,330,MPI_COMM_WORLD,status,ierr)
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  ...
  ! send to the down recieve from the up
  call MPI_Sendrecv(u(0:nrl+1,1),nrl+2,MPI_DOUBLE_PRECISION,p_down,127,&
       u(0:nrl+1,nsl+1),nrl+2,MPI_DOUBLE_PRECISION,p_up,127,MPI_COMM_WORLD,status,ierr)
  ! send to the up recieve from the down
  call MPI_Sendrecv(u(0:nrl+1,nsl),nrl+2,MPI_DOUBLE_PRECISION,p_up,129,&
       u(0:nrl+1,0),nrl+2,MPI_DOUBLE_PRECISION,p_down,129,MPI_COMM_WORLD,status,ierr)
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  ...
  ! Allocate the force, rhside, and Laplacian arrays.
  ...
  do it = 1,nsteps
    ...
    call compute_lap(Lap,u,a,jac,rx,sx,ry,sy,nxl,nyl,hr,hs,r,s,t) !TODO Take out those last parameters.
  ...
  t = dt*dble(it) ! t final is dt*(nsteps + 1)
  call set_bc(u,nxl,nyl,x,y,t,manutype,p_left,p_right,p_up,p_down)
  !write(*,*) "Approximation at (", x(10,10), ",", y(10,10), ",", t, ")=", u(10,10)
  !write(*,*) "Actual at (", x(10,10), ",", y(10,10), ",", t, ")=", sin(2*(dt*dble(it)) + acos(-1.d0) - x(10,10))*sin(y(10,10))
  call get_error(u, jac, nrl, nsl, error, x, y, t, manutype)
  call get_max(jmax,nxl,nyl,jac)
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)

  call MPI_REDUCE(error,max_error,1,MPI_DOUBLE_PRECISION,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(jmax,true_jmax,1,MPI_DOUBLE_PRECISION,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  if (myid == 0) then
    do i = 0,nxl+1
      do j = 0,nyl+1
        ...
        write(*,*) "xr=", xr(i,j), "xs=", xs(i,j)
        write(*,*) "yr=", yr(i,j), "ys=", ys(i,j)
        write(*,*) "u approx=", u(i,j)
        write(*,*) "u exact=", sin(2*t + acos(-1.d0) - x(i,j))*sin(y(i,j))
        write(*,*) "error=", abs(sin(2*t + acos(-1.d0) - x(i,j))*sin(y(i,j)) - u(i,j))
        write(*,*) "============================"
      end do
    end do
    !write(*,*), hr*hs*true_jmax, " ", max_error, t
  endif

  deallocate(r,s,u,up,um,ur,us)
  deallocate(x,y)
  deallocate(xr,yr,xs,ys,rx,ry,sx,sy,jac,a)
  deallocate(Force,rhside,utt,Lap)
end subroutine computeerror

subroutine get_max(jmax,nxl,nyl,jac)
  implicit none
  integer, intent(in) :: nxl,nyl
  real(kind = 8), dimension(0:nxl+1,0:nyl+1), intent(in) :: jac
  real(kind = 8), intent(inout) :: jmax

  integer :: i,j
  
  do i = 0,nxl+1
    do j = 0,nyl+1
      if (jmax < jac(i,j)) then
        jmax = jac(i,j)
      endif
    end do
  end do
...
subroutine compute_lap(Lap,u,a,jac,rx,sx,ry,sy,nxl,nyl,hr,hs,r,s,t) !TODO: Delete last t parameter.
  implicit none
  integer, intent(in) :: nxl,nyl
  ...
  real(kind = 8), dimension(1:nxl), intent(in) :: r
  real(kind = 8), dimension(1:nyl), intent(in) :: s
  real(kind = 8), intent(in) :: t
  ...
  do i = 1,nxl
    do j = 1,nyl
      ...
      !term(i,j,1) = s(j)*cos(r(i)-2*t)*sin(s(j)) + r(i)*s(j)*sin(2*t - r(i))*sin(s(j))
      ...
      !term(i,j,8) = r(i)*sin(r(i)-2*t)*cos(s(j)) + r(i)*s(j)*sin(2*t-r(i))*sin(s(j))
    end do
  end do
...
subroutine get_error(v, jac, nrl, nsl, error, x, y, t, error_type)
  implicit none
  integer, intent(in) :: nrl,nsl,error_type
  real(kind = 8), intent(in) :: v(0:nrl+1,0:nsl+1), x(0:nrl+1,0:nsl+1), y(0:nrl+1,0:nsl+1)
  ...
  allocate(u(0:nrl+1, 0:nsl+1))

  if (error_type == 0) then
    !u = sin(omega*(t+t_0)-k_x*x)*sin(k_y*y)
    u = sin(2.d0*t + pi - x)*sin(y)
  endif
  call curvierror(u, v, jac, nrl, nsl, error)
  deallocate(u)
...
subroutine curvierror(u, v, jac, nrl, nsl, error)
  ...
  error = (error * (2.d0/dble(nrl)))**(0.5d0)
  deallocate(interior)
