program weakanalysis
  use mpi
  implicit none
  integer :: ierr, nprocs, myid
  integer :: n
  real(kind = 8) :: start_time, end_time, time_sum
  ...
  ! Compute the time to complete 1000 steps with a grid whose number of gridpoints is proportional
  ! to the number of processes.
  do n = 1,num_iterations
    start_time = MPI_Wtime()
    call timing(20*nprocs,80,1.d0/dble(1000+1),1000,myid,nprocs,ierr)
    end_time = MPI_Wtime()
    time_sum = time_sum + (end_time - start_time)
  end do
  if (myid == 0) then
    write(*,*) nprocs, " ", (time_sum / dble(num_iterations))
  endif
  call mpi_finalize(ierr)
end program weakanalysis

subroutine timing(nx,ny,dt,nsteps,myid,nprocs,ierr)
  use mpi
  use xycoord
  implicit none
  ...
  do it = 1,nsteps
    ...
    call compute_lap(Lap,u,a,jac,rx,sx,ry,sy,nxl,nyl,hr,hs)
...
subroutine compute_lap(Lap,u,a,jac,rx,sx,ry,sy,nxl,nyl,hr,hs) !TODO: Delete last t parameter.
  implicit none
